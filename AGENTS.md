...
- **GLEW on Linux**: The GLEW commit used by OpenRV (`e1a80a9f...`) has duplicate variable definitions in `src/glew.c` (lines 2059-2068). We apply a build-time patch to `cmake/dependencies/glew.cmake` that adds a `PATCH_COMMAND` to remove the duplicates. The path to `glew.c` must be `<SOURCE_DIR>/glew-${_version}/src/glew.c` because the GitHub archive extracts to a top-level folder `glew-<version>`. Patch: `ci/linux/patches/glew_fix_duplicates.patch`.
