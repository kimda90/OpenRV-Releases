...
- **Build script**: Our `ci/linux/build_in_container.sh` is mounted into the container and handles cloning, patching, building, and packaging. It preserves the `_build` directory during cloning when it's a Docker bind mount from the cache, using a temporary clone directory and carefully moving contents while avoiding the mount point.
