...
- **Windows build-time tweaks (Configure phase)**: We edit the cloned OpenRV `cmake/dependencies/` files via **PowerShell search/replace** (no `git apply`), so changes work across OpenRV versions. (1) **DAV1D**: add `-Denable_asm=false` to the Meson CONFIGURE_COMMAND in `dav1d.cmake`. (2) **FFmpeg**: insert `--toolchain=msvc` immediately after `${_configure_command}` in `ffmpeg.cmake` so FFmpeg sees the MSVC toolchain first and avoids "Unsupported MSVC version". (3) **atomic_ops**: wrap the autoconf CONFIGURE_COMMAND in `atomic_ops.cmake` with `cmake -E env CC=gcc CXX=g++` so the configure step uses gcc (from MSYS2); with `CC=cl` the autoconf "C compiler cannot create executables" test fails. Legacy patch files in `ci/windows/patches/` are unused; the script no longer applies them.
