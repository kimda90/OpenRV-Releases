...
- **`ci/linux/`** – Linux builds: `Dockerfile.ubuntu22.04`, `Dockerfile.rocky9-cy2024` (extended image), `build_in_container.sh`, `detect_qt.sh`, `patches/dav1d_use_git.patch`, `patches/dav1d_use_git_oneline.patch`, `patches/glew_fix_duplicates.patch`, `KNOWN_ISSUES.md`
...
- **GLEW on Linux**: The GLEW commit used by OpenRV (`e1a80a9f...`) has duplicate variable definitions in `src/glew.c` (lines 2059-2068). We apply a build-time patch to `cmake/dependencies/glew.cmake` that adds a `PATCH_COMMAND` to remove the duplicates. The patch uses `find <SOURCE_DIR> -name glew.c -type f -path "*/src/glew.c" -exec sed ...` so it works regardless of how the archive extracts (with or without a top-level `glew-<version>` folder). Patch: `ci/linux/patches/glew_fix_duplicates.patch`.
